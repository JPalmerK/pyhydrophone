...
class custom(Hydrophone):
    ...
    def __init__(self, name, model, serial_number, 
        ...
        """
        Init an instance of custom recoding system.
        ...
        """
        super().__init__(name, model, serial_number, sensitivity, preamp_gain, 
                         Vpp, string_format, calibration_file, **kwargs)
        # # ✅ Explicitly load the calibration file
    ...
    def calibrate(self, file_path, zip_mode=False):
        return 0  # skip trying to read WAV header
    def get_freq_cal(self, val='sensitivity', sep=';', 
                     freq_col_id=0, val_col_id=1, 
                     start_data_id=0):
        super().get_freq_cal(sep=sep, 
                             freq_col_id=freq_col_id, 
                             val_col_id=val_col_id, 
